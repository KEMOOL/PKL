...
(function(e, i) {
		...
		if (window.File && window.FileReader && window.FileList && window.Blob) {
			var s = {
				...
				messages: {
					...
					replace: "seret dan lepas file disini atau klik untuk mengganti file",
					remove: "Hapus",
					error: "Maaf, anda mengunggah file yang salah"
				},
				error: {
					fileSize: "Mohon unggah file kurang dari 2MB.",
					minWidth: "The image width is too small ({{ value }}}px min).",
					maxWidth: "The image width is too big ({{ value }}}px max).",
					minHeight: "The image height is too small ({{ value }}}px min).",
					maxHeight: "The image height is too big ({{ value }}px max).",
					imageFormat: "Mohon unggah file JPG, JPEG, PNG & GIF",
					fileExtension: "Mohon unggah file JPG, JPEG, PNG & GIF"
				},
				tpl: {
					wrap: '<div class="card card-body view file-upload"></div>',
					loader: '<div class="mask rgba-stylish-slight"></div>',
					message:
						'<div class="card-text file-upload-message"><i class="fas fa-cloud-upload-alt fa-4x"></i><p>{{ default }}</p></div>',
					preview:
					...
					clearButton:
						'<button type="button" class="btn btn-sm btn-danger" id="hapusFile">Hapus <i class="far fa-trash-alt fa-2x ml-1"></i></button>',
	...
	return (
		...
		(i.prototype.setPreview = function(i, t) {
			...
			if ((this.hideLoader(), i === !0)) {
				...
				e("<i />")
					...
					.appendTo(s),
					e('<span class="file-upload-extension" />')
		...
		(i.prototype.resetPreview = function() {
			...
			e.find(".file-upload-extension").remove(),
				e.find("i").remove(),
				e.find(".file-upload-preview-img").remove(),
				this.preview.hide(),
				this.showLoader();
		}),
			...
			if (0 === this.errorsEvent.errors.length) {
				...
				this.input.trigger(i, [this]),
					i.result !== !1 &&
						...
						this.input.val(""),
						this.resetPreview(),
						this.input.trigger(e.Event("file_upload.afterClear"), [this]));
